...
export async function searchMemories(
  ...
  return searchPoints(COLLECTIONS.MEMORIES, vector, limit, excludeSystemAndTrashed(filter), scoreThreshold);
}

export async function updatePayload(collection: string, id: string, payload: Record<string, unknown>): Promise<void> {
  const qdrant = getQdrantClient();
...
export async function scrollPoints(collection: string, filter?: Record<string, unknown>, limit = 20, offset?: string) {
  const qdrant = getQdrantClient();
