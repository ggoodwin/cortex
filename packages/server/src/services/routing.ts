...
import { COLLECTIONS, SYSTEM_POINT_ID } from '@cortex/shared'
import type { RoutingRulePayload, RoutingPresetPayload, RoutingPreset, RoutingRule, RoutingResolveInput, RoutingResolveResult } from '@cortex/shared'
import { generateEmbedding } from './embeddings.js'
import { upsertPoint, getPoint, searchPoints, scrollPoints, deletePoint, updatePayload, getQdrantClient } from './qdrant.js'
...
export async function setActivePreset(name: string): Promise<boolean> {
  ...
  const all = await scrollPoints(COL, {
    must: [{ key: '_type', match: { value: 'preset_meta' } }],
  }, 100)
...
export async function createOrReplacePreset(
  ...
  const existing = await scrollPoints(COL, {
  ...
  try { await deletePoint(COL, presetMetaId(name)) } catch { /* may not exist */ }
