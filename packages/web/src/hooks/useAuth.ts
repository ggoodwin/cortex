...
export function useAuth() {
  ...
  const login = async (username: string, password: string) => {
    const res = await api.post<{ ok: boolean; data: { token: string; username: string } }>('/auth/login', { username, password })
    setToken(res.data.token)
    setState((s) => ({ ...s, username: res.data.username }))
  }

  const setup = async (username: string, password: string) => {
    const res = await api.post<{ ok: boolean; data: { token: string; username: string } }>('/auth/setup', { username, password })
    setToken(res.data.token)
    setState((s) => ({ ...s, username: res.data.username }))
  }
