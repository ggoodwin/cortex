...
export function MemoryEditor() {
  ...
  useEffect(() => {
    if (!id) return
    api.get<{ ok: boolean; data: Memory }>(`/memory/${id}`)
      .then((r) => {
        const m = r.data
        setMemory(m)
    ...
    <div className="max-w-2xl">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={save} disabled={saving} className="px-4 py-1.5 bg-blue-600 hover:bg-blue-700 rounded text-sm font-medium disabled:opacity-50">
            {saving ? 'Saving...' : 'Save'}
          </button>
        </div>
      </div>
